...
export function Mug(props) {
  ...
  const [baseMaterial, setBaseMaterial] = React.useState(null)
  React.useEffect(() => {
    const newBaseMaterial = materials.default.clone()
    newBaseMaterial.color.setHSL(
      Math.random(), Math.random(), Math.random() * 0.25 + 0.5
    )
    setBaseMaterial(newBaseMaterial)
  }, [materials.default])

  return (
    <group ref={cup} {...bind} dispose={null}>
      <group rotation={[Math.PI / 2, 0, 0]} scale={[0.012, 0.012, 0.012]}>
        <mesh receiveShadow castShadow material={baseMaterial || materials.default} geometry={nodes['buffer-0-mesh-0'].geometry} />
